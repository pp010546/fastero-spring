	...
	public Result register(UserVO vo) {
		...
		try {
			...
			if (account == null || "".equals(account))
				return R.fail("帳號不可為空！");
			// 1-1. 檢查帳號是否為email
			if (!Regex.emailCheck(account))
				return R.fail("email格式錯誤！");
			// 1-2. 檢查帳號是否存在
			if (allAccount.contains(account))
				return R.fail("帳號已存在！");
			// 2.檢查密碼
			if (password == null || "".equals(password))
				return R.fail("密碼不可為空！");
			// 2-1. 檢查密碼格式
			if (!Regex.passwordCheck(password))
				return R.fail("密碼格式錯誤！");
			// 3. 檢查姓名
			if (name == null || "".equals(name))
				return R.fail("姓名不可為空！");
			// 3-1. 檢查姓名格式
			if (!Regex.nameCheck(name))
				return R.fail("姓名格式錯誤！");
			// 4. 檢查電話
			if (phone == null || "".equals(phone))
				return R.fail("電話不可為空！");
			// 4-1 檢查電話格式
			if (!Regex.phoneCheck(phone))
				return R.fail("電話格式錯誤！");
			// 4-2 檢查電話是否重複
			if (allPhone.contains(phone))
				return R.fail("此電話已被使用！");
